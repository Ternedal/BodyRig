...
if ($head -ceq $BaselineRevision) {
    throw "Candidate runner refuses the uncapped baseline authority. Switch to the sampled candidate checkout first."
}
...
if ($dirty.Count -gt 0) {
    throw "Candidate checkout is dirty. Refusing to start physical A/B candidate."
}
...
if ($LASTEXITCODE -ne 0 -or $samplingRevision -cne $ExpectedSamplingRevision) {
    throw "Candidate recovery sampling authority mismatch. Expected $ExpectedSamplingRevision, got $samplingRevision."
}

Assert-RunningServiceAuthority -Root $RepoRoot -Revision $head
...
if ($jobRevision -cne $head) {
    throw "Started job is not bound to the exact candidate checkout. Expected $head, got $jobRevision."
}

Write-Host "BodyRig recovery throughput A/B candidate: STARTED"
Write-Host "Person:    $PersonId"
Write-Host "Performer: $performerId"
Write-Host "Job:       $jobId"
Write-Host "Revision:  $jobRevision"
Write-Host "Sampling:  $samplingRevision"
...
if (-not (Test-Path -LiteralPath $watch -PathType Leaf)) {
    throw "Canonical body-build monitor is missing from candidate checkout: $watch"
}
...
if ($finalStatus -ne "succeeded") {
    ...
    Write-Host "BodyRig recovery throughput A/B candidate: FAILED"
    Write-Host "Job:      $jobId"
