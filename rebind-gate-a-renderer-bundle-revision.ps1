...
if ([System.Environment]::OSVersion.Platform -ne [System.PlatformID]::Win32NT) {
    throw "Renderer bundle revision rebind is Windows-only."
}
if ($PSVersionTable.PSVersion.Major -lt 7) {
    throw "PowerShell 7+ (pwsh) is required for renderer bundle revision rebind."
}

$repoRoot = (Resolve-Path $PSScriptRoot).Path
$SourceAcceptanceDir = Need-Directory -Path $SourceAcceptanceDir -Label "Source reconciled Gate A acceptance directory"
$OutputDir = [IO.Path]::GetFullPath($OutputDir)
if (Test-Path -LiteralPath $OutputDir) { throw "Renderer-bundle-rebound Gate A output already exists; refusing cross-attempt reuse: $OutputDir" }
...
if ($dirty.Count -gt 0) { throw "BodyRig checkout is dirty; renderer bundle revision rebind requires an exact clean checkout." }
...
$sourceAcceptance = Read-Json -Path $sourceAcceptancePath -Label "Source reconciled Gate A acceptance report"
...
foreach ($forbidden in @(
    ...
    if (Test-Path -LiteralPath (Join-Path $SourceAcceptanceDir $forbidden)) {
        throw "Renderer bundle rebind must happen before physical renderer evidence exists: $forbidden"
    }
}

& git -C $repoRoot cat-file -e "$ExpectedSourceRevision^{commit}" 2>$null
if ($LASTEXITCODE -ne 0) { throw "Source reconciliation revision is not present in the local Git object database." }
& git -C $repoRoot merge-base --is-ancestor $ExpectedSourceRevision $head
if ($LASTEXITCODE -ne 0) { throw "Current HEAD is not a descendant of the source reconciliation revision." }

$deltaLines = @(& git -C $repoRoot diff --name-status "$ExpectedSourceRevision..$head")
if ($LASTEXITCODE -ne 0) { throw "Could not inspect renderer bundle revision delta." }
$deltaNames = @()
foreach ($line in $deltaLines) {
    if ([string]::IsNullOrWhiteSpace([string]$line)) { continue }
    $parts = ([string]$line) -split "`t"
    if ($parts.Count -ne 2 -or $parts[0] -notin @("A", "M")) { throw "Unsupported renderer bundle rebind revision delta: $line" }
    $deltaNames += $parts[1]
}
...
if (@(Compare-Object -ReferenceObject $expectedNames -DifferenceObject $deltaNames).Count -ne 0) {
    throw "Revision delta is broader than the approved cumulative renderer-only repair/rebind set: $($deltaNames -join ', ')"
}
...
$sessionHash = Need-Sha256 -Value ([string]$sourceAcceptance.physical_clone.session_sha256) -Label "physical_clone.session_sha256"
$readinessHash = Need-Sha256 -Value ([string]$sourceAcceptance.physical_clone.readiness_sha256) -Label "physical_clone.readiness_sha256"
$skinQaHash = Need-Sha256 -Value ([string]$sourceAcceptance.skin_qa.report_sha256) -Label "skin_qa.report_sha256"
$reconciliationHash = Need-Sha256 -Value ([string]$sourceAcceptance.physical_clone.reconciliation_sha256) -Label "physical_clone.reconciliation_sha256"

$sessionPath = Need-File -Path (Join-Path $SourceAcceptanceDir "bodyrig-physical-clone-session.json") -Label "Original physical clone session evidence"
$readinessPath = Need-File -Path (Join-Path $SourceAcceptanceDir "bodyrig-rig-readiness.json") -Label "Physical clone readiness evidence"
$skinQaPath = Need-File -Path (Join-Path $SourceAcceptanceDir "bodyrig-skin-qa.json") -Label "Skin QA evidence"
$reconciliationPath = Need-File -Path (Join-Path $SourceAcceptanceDir "bodyrig-physical-clone-reconciliation.json") -Label "Physical clone reconciliation evidence"
if ((Sha256 $sessionPath) -ne $sessionHash) { throw "Source physical clone session bytes no longer match Gate A." }
if ((Sha256 $readinessPath) -ne $readinessHash) { throw "Source readiness bytes no longer match Gate A." }
if ((Sha256 $skinQaPath) -ne $skinQaHash) { throw "Source skin QA bytes no longer match Gate A." }
if ((Sha256 $reconciliationPath) -ne $reconciliationHash) { throw "Source reconciliation bytes no longer match Gate A." }

$sourcePayloadHashes = [ordered]@{}
foreach ($payloadName in @("avatar.vrm", "bodyprint.json")) {
    $payloadPath = Need-File -Path (Join-Path (Join-Path $SourceAcceptanceDir "runtime") $payloadName) -Label "Source runtime $payloadName"
    $sourcePayloadHashes[$payloadName] = Sha256 $payloadPath
}

$sourceAcceptanceHash = Sha256 $sourceAcceptancePath
$parent = Split-Path -Parent $OutputDir
if (-not (Test-Path -LiteralPath $parent -PathType Container)) { throw "Renderer-bundle-rebound output parent does not exist: $parent" }
$attempt = Join-Path $parent (".bodyrig-renderer-bundle-rebind-" + [Guid]::NewGuid().ToString("N"))
$committed = $false
try {
    ...
    if ($copiedPackage.Count -ne 1 -or (Sha256 $copiedPackage[0].FullName) -ne $ExpectedPackageSha256) { throw "Renderer bundle rebind changed package bytes." }
    $copiedRuntimePath = Need-File -Path (Join-Path (Join-Path $attempt "runtime") "runtime-manifest.json") -Label "Copied runtime manifest"
    if ((Sha256 $copiedRuntimePath) -ne $sourceRuntimeHash) { throw "Renderer bundle rebind changed runtime-manifest bytes." }
    foreach ($payloadName in @("avatar.vrm", "bodyprint.json")) {
        $copiedPayload = Need-File -Path (Join-Path (Join-Path $attempt "runtime") $payloadName) -Label "Copied runtime $payloadName"
        if ((Sha256 $copiedPayload) -ne [string]$sourcePayloadHashes[$payloadName]) { throw "Renderer bundle rebind changed runtime payload bytes: $payloadName" }
    }
    foreach ($evidence in @(
        @{ Name="bodyrig-physical-clone-session.json"; Hash=$sessionHash },
        @{ Name="bodyrig-rig-readiness.json"; Hash=$readinessHash },
        @{ Name="bodyrig-skin-qa.json"; Hash=$skinQaHash },
        @{ Name="bodyrig-physical-clone-reconciliation.json"; Hash=$reconciliationHash }
    )) {
        $copiedEvidence = Need-File -Path (Join-Path $attempt $evidence.Name) -Label ("Copied " + $evidence.Name)
        if ((Sha256 $copiedEvidence) -ne [string]$evidence.Hash) { throw "Renderer bundle rebind changed evidence bytes: $($evidence.Name)" }
    }

    $rebind = [ordered]@{
        format = "bodyrig-renderer-bundle-revision-rebind"
        version = 1
        ...
        physical_evidence_bytes_preserved = $true
        recovery_rerun = $false
        clone_rerun = $false
    }
    $rebindPath = Join-Path $attempt "bodyrig-renderer-bundle-revision-rebind.json"
    ...
    $acceptance.physical_clone | Add-Member -NotePropertyName renderer_bundle_revision_rebind_sha256 -NotePropertyValue $rebindHash -Force
    $acceptance.physical_clone | Add-Member -NotePropertyName renderer_bundle_revision_source_bodyrig_revision -NotePropertyValue $ExpectedSourceRevision -Force

    $tempAcceptance = Join-Path $attempt (".bodyrig-acceptance-renderer-bundle-rebind-" + [Guid]::NewGuid().ToString("N") + ".tmp")
    try {
    ...
    $check = Read-Json -Path $acceptancePath -Label "Renderer-bundle-rebound Gate A acceptance report"
    if (([string]$check.bodyrig_revision).ToLowerInvariant() -ne $head) { throw "Renderer-bundle-rebound acceptance revision did not persist." }
    if (([string]$check.package.package_sha256).ToLowerInvariant() -ne $ExpectedPackageSha256) { throw "Renderer-bundle-rebound acceptance changed package identity." }
    if (([string]$check.runtime.manifest_sha256).ToLowerInvariant() -ne $sourceRuntimeHash) { throw "Renderer-bundle-rebound acceptance changed runtime identity." }
    if (([string]$check.physical_clone.session_sha256).ToLowerInvariant() -ne $sessionHash) { throw "Renderer-bundle-rebound acceptance changed original session identity." }
    if (([string]$check.physical_clone.reconciliation_sha256).ToLowerInvariant() -ne $reconciliationHash) { throw "Renderer-bundle-rebound acceptance changed reconciliation identity." }
    if (([string]$check.physical_clone.renderer_bundle_revision_rebind_sha256).ToLowerInvariant() -ne $rebindHash) { throw "Renderer-bundle-rebound acceptance is not bound to rebind evidence." }
...
if ($LASTEXITCODE -ne 0 -or $dirtyAfter.Count -gt 0) { throw "Renderer bundle revision rebind did not preserve a clean BodyRig checkout." }
$currentHeadLines = @(& git -C $repoRoot rev-parse HEAD 2>&1)
if ($LASTEXITCODE -ne 0 -or $currentHeadLines.Count -ne 1) { throw "Could not re-check BodyRig HEAD after renderer bundle rebind." }
$currentHead = ([string]$currentHeadLines[0]).Trim().ToLowerInvariant()
if ($currentHead -ne $head) { throw "BodyRig HEAD changed during renderer bundle revision rebind." }

Write-Host "BodyRig cumulative renderer-only revision rebind: PASS"
