...
if ($dirty.Count -ne 0) {
    throw "Identity recognizer A/B diagnostic requires a clean BodyRig checkout."
}
...
$tool = Need-File (Join-Path $script:RepoRoot "tools\photoreal_identity_recognizer_ab_diagnostic.py") "Identity recognizer A/B diagnostic"
...
try {
    $tempIdentityRequest = New-WslTransportRequest -SourceRequest $identityRequest -Prefix "identity-recognizer-ab"
    $tempCalibrationRequest = New-WslTransportRequest -SourceRequest $calibrationRequest -Prefix "calibration-recognizer-ab"

    $stamp = Get-Date -Format "yyyyMMdd-HHmmss"
    $output = Join-Path $RunDirectory ("identity-recognizer-ab-diagnostic-{0}.json" -f $stamp)
    if (Test-Path -LiteralPath $output) {
        throw "Identity recognizer A/B diagnostic output already exists: $output"
    }
    ...
    Write-Host "============================================================"
    Write-Host "BODYRIG PHOTOREAL V2 - IDENTITY RECOGNIZER A/B DIAGNOSTIC"
    ...
    Write-Host "Alternate:      antelopev2 / glintr100 / ResNet100@Glint360K"
    ...
    & wsl.exe @wslArgs
    if ($LASTEXITCODE -ne 0) {
        throw "Identity recognizer A/B diagnostic failed with exit code $LASTEXITCODE."
    }

    $result = Get-Content -LiteralPath $output -Raw -Encoding UTF8 | ConvertFrom-Json -Depth 100

    Write-Host ""
    Write-Host "Identity recognizer A/B diagnostic: PASS"
    ...
    foreach ($name in @("bank-w600k-r50","replay-w600k-r50","antelopev2-glintr100")) {
        $variant = $result.variants.$name
        if ($null -eq $variant) { continue }
        Write-Host ("{0,-24} pos={1,6:P1} neg={2,6:P1} status={3}" -f $name, [double]$variant.positive_reference_coverage, [double]$variant.negative_observation_coverage, $variant.scoring_status)
