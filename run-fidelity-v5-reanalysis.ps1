...
$stagingRoot = Join-Path $WorkRoot (".reanalysis-v5-$tag-" + [guid]::NewGuid().ToString("N") + ".tmp")
New-Item -ItemType Directory -Path $stagingRoot | Out-Null
$published = $false

try {
    $evaluations = @(
        (Join-Path $stagingRoot "iteration-01-baseline.json")
        (Join-Path $stagingRoot "iteration-02-refit1.json")
        (Join-Path $stagingRoot "iteration-03-reconstruction2.json")
    )
    ...
    $decision = Join-Path $stagingRoot "convergence-decision.json"
    Write-Host ""
