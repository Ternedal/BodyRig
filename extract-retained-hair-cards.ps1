...
if ([System.Environment]::OSVersion.Platform -ne [System.PlatformID]::Win32NT) { throw "BodyRig retained hair-card extraction is Windows/WSL-only." }
...
if ($LASTEXITCODE -ne 0 -or $dirty.Count -gt 0) { throw "Retained hair-card extraction requires an exact clean BodyRig checkout." }
...
$DonorObj = Need-File -Path $DonorObj -Label "Hair-card donor OBJ"
$extractScript = Need-File -Path (Join-Path $repoRoot "bodyrig\bridges\sith_source_hair_cards.py") -Label "BodyRig source-guided hair-card bridge"

$OutputDir = [IO.Path]::GetFullPath($OutputDir)
if (Test-Path -LiteralPath $OutputDir) { throw "Hair-card output already exists: $OutputDir" }
$outputParent = Split-Path -Parent $OutputDir
if ([string]::IsNullOrWhiteSpace($outputParent)) { throw "Hair-card output must have a parent directory." }
...
if ([string]::IsNullOrWhiteSpace($InstallRoot)) {
    ...
    if ($wslHomeProbe.ExitCode -ne 0 -or [string]::IsNullOrWhiteSpace($wslHomeProbe.Text)) { throw "Could not resolve WSL home directory: $($wslHomeProbe.Text)" }
...
Write-Host "BodyRig retained source-guided hair cards"
...
Write-Host "Raw shell runtime: FALSE"
...
if ($run.ExitCode -ne 0) { throw "Source-guided hair-card extraction failed with exit code $($run.ExitCode)." }

if ((Sha256 $reconstruction) -ne $reconstructionShaBefore -or (Sha256 $sourceMesh) -ne $sourceShaBefore) {
    throw "Retained reconstruction/source bytes changed during hair-card extraction."
}
$evidencePath = Need-File -Path (Join-Path $OutputDir "source-hair-candidate.json") -Label "Source-guided hair-card evidence"
$hairObj = Need-File -Path (Join-Path $OutputDir "hair_source.obj") -Label "Source-guided hair-card OBJ"
try { $evidence = Get-Content -LiteralPath $evidencePath -Raw -Encoding UTF8 | ConvertFrom-Json -Depth 20 }
catch { throw "Source-guided hair-card evidence is unreadable." }
if ([string]$evidence.format -ne "bodyrig-source-hair-candidate" -or -not (Test-V1Version $evidence.version) -or
    [string]$evidence.method -ne "retained-sith-source-guided-hair-cards-v1" -or
    $evidence.sourceDerived -ne $true -or $evidence.generativeGeometry -ne $true -or
    ...
    throw "Source-guided hair-card evidence violates the review-only authority boundary."
}
if ([string]$evidence.sourceReconstructionSha256 -ne $reconstructionShaBefore -or
    ...
    throw "Source-guided hair-card evidence does not bind exact retained/output bytes."
}

Write-Host ""
Write-Host "BodyRig retained source-guided hair cards: CANDIDATE PASS"
...
Write-Host "Guide p95:      $([double]$evidence.sourceToDonorDistanceP95)"
Write-Host "Geometry:       OPEN SOURCE-GUIDED CARDS"
