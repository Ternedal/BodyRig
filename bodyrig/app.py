...
def body_library() -> Path:
    ...
    if root: return Path(root).expanduser().resolve() / "bodies"
...
def health() -> dict: return {"ok": True, "service":"bodyrig", "version":__version__}


@app.get("/api/v1/bodies")
def list_bodies() -> dict:
    library=body_library(); bodies=[]
    if library.exists():
        for path in sorted(library.glob("*.mrbody")):
            try: validated=validate_package(path)
            except MRBodyError: continue
...
def import_body(request: ImportRequest) -> dict:
    try:
        target=install_package(request.path,body_library()); validated=validate_package(target)
    except (MRBodyError,OSError) as exc: raise HTTPException(status_code=400,detail=str(exc)) from exc
...
def activate_body(body_id: str) -> dict:
    try: validated=validate_package(body_library()/f"{body_id}.mrbody")
    except (MRBodyError,OSError) as exc: raise HTTPException(status_code=404,detail="body not found or invalid") from exc
    return runtime.activate(validated.manifest["id"]).__dict__


@app.post("/api/v1/runtime/cue")
def apply_cue(cue: BodyCue) -> dict: return runtime.apply_cue(cue).__dict__


@app.post("/api/v1/runtime/speech-timing")
def apply_speech_timing(timing: SpeechTiming) -> dict:
    try: return runtime.apply_speech(timing).__dict__
    except ValueError as exc: raise HTTPException(status_code=409,detail=str(exc)) from exc


@app.get("/api/v1/runtime/state")
def runtime_state() -> dict: return runtime.snapshot().__dict__


def run() -> None:
    import uvicorn
    host=os.environ.get("BODYRIG_HOST",DEFAULT_HOST)
    if host not in {"127.0.0.1","localhost","::1"} and os.environ.get("BODYRIG_ALLOW_REMOTE") != "1": raise SystemExit("BodyRig refuses non-loopback bind unless BODYRIG_ALLOW_REMOTE=1")
