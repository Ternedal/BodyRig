...
from .wardrobe_authority import CHECKLIST_FIELDS, WardrobeAuthorityError, authority_dir, write_authority
...
def _read_json(path: str, label: str) -> dict:
    ...
    try:
        ...
        raise WardrobeAuthorityError(f"{label} is unreadable: {source}") from exc
    if not isinstance(value, dict):
        raise WardrobeAuthorityError(f"{label} must contain a JSON object")
    return value
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description=(
            "Record a create-only operator-supplied wardrobe authority bound to exact source presentation, "
            "exact package/source-geometry lineage, exact wardrobe renders and exact deformation evidence."
        )
    )
    ...
    parser.add_argument("--confirm-wardrobe-checklist", action="store_true")
    parser.add_argument("--confirm-footwear-review", action="store_true")
    parser.add_argument("--quality-note", required=True)
    args = parser.parse_args(argv)
    try:
        if not args.confirm_wardrobe_checklist:
            raise WardrobeAuthorityError("wardrobe authority requires explicit --confirm-wardrobe-checklist")
        ...
        receipt = write_authority(
            ...
            footwear_review_passed=args.confirm_footwear_review,
            quality_note=args.quality_note,
        )
        root = authority_dir(person_library(), str(receipt["person_id"]), str(receipt["person_revision"]), str(receipt["review_id"]))
    except (WardrobeAuthorityError, OSError, ValueError) as exc:
        print(f"BodyRig wardrobe authority: FAIL: {exc}", file=sys.stderr)
        return 1
    print(json.dumps({
        "ok": True,
