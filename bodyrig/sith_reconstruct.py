...
from .sith_preflight import (
    PINNED_BLOBS,
    SITH_CENTRALIZE_RGBA_BLOB,
    SITH_REVISION,
)
...
def load_prepared_input(workspace: str | Path) -> tuple[Path, dict[str, Any], str]:
    ...
    prep_raw, prep = _read_json(prep_path, label="SiTH prepared-input evidence")
    required = {
        "format",
        "version",
        ...
        "sith_revision",
        "centralizer_blob",
        "centralized_image_sha256",
        "openpose_keypoints_sha256",
        "centralized_size",
        "openpose_quality",
    }
...
def validate_reconstruction_outputs(stage: Path) -> dict[str, str]:
    ...
    return {
        "smplx_obj_sha256": _sha256(stage / "smplx" / "000_smplx.obj"),
        "back_image_sha256": _sha256(stage / "back_images" / "000_000.png"),
...
def reconstruct_sith(
    *,
    ...
    if model["sha256"] != diffusion_model_sha256:
        raise SithReconstructError(
            f"SiTH diffusion model tree digest mismatch: expected {diffusion_model_sha256}, got {model['sha256']}"
        )
    ...
    _checked_wsl(
        ...
        command=[
            python,
            "fit.py",
            "-i", f"{linux_stage}/images",
            "-o", f"{linux_stage}/smplx",
            "--size", "1024",
            "--opt_orient",
            "--opt_betas",
        ],
    ...
    if sorted(path.name for path in (stage / "smplx").iterdir()) != ["000_smplx.obj"] or smplx_obj.stat().st_size < 64:
        raise SithReconstructError("SiTH fit must produce exactly smplx/000_smplx.obj")
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        return 1
    print(
        "BodyRig SiTH reconstruction: PASS | "
        f"model={evidence['diffusion_model_sha256'][:12]} | seed={evidence['seed']} | UV mesh ready"
    )
    return 0
