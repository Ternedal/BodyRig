...
def compare_runs(
    ...
    *,
    expected_baseline_bodyrig_revision: str,
    ...
    expected_baseline = _bodyrig_revision(
        expected_baseline_bodyrig_revision, "expected baseline BodyRig revision"
    )
    expected_candidate = _bodyrig_revision(
        expected_candidate_bodyrig_revision, "expected candidate BodyRig revision"
    )
    if baseline_bodyrig_revision != expected_baseline:
        blockers.append("baseline BodyRig revision does not match the explicitly selected baseline authority")
    if candidate_bodyrig_revision != expected_candidate:
    ...
    software_authority_bound = (
        baseline_bodyrig_revision == expected_baseline
        and candidate_bodyrig_revision == expected_candidate
    )
...
def audit(
    ...
    *,
    ...
    return compare_runs(
        ...
        expected_baseline_bodyrig_revision=expected_baseline_bodyrig_revision,
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="Read-only exact-revision A/B audit for the versioned BodyRig PHALP sampling candidate. Never grants promotion authority."
    )
    ...
    parser.add_argument(
        "--baseline-bodyrig-revision",
        required=True,
        help="exact 40-character BodyRig revision that produced the selected uncapped baseline job",
    )
    parser.add_argument(
    ...
    try:
        result = audit(
            args.baseline_job,
            args.candidate_job,
            expected_baseline_bodyrig_revision=args.baseline_bodyrig_revision,
