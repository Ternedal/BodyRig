...
def run_preflight(
    *,
    ...
    completed = _run_wsl(
        wsl_exe=wsl_exe,
        distribution=distribution,
        command=[python, "-c", _python_probe_script(repo, openpose)],
    )
    if completed.returncode != 0:
        errors.append(f"SiTH Python probe failed: {completed.stderr.strip()[-1500:]}")
    else:
        try:
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="Fail-closed preflight for the pinned experimental SiTH BodyRig adapter."
    )
