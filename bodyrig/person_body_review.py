...
def validate_fidelity_output(
    ...
    *,
    ...
    if comparison.get("format") != COMPARISON_FORMAT or not _is_version(comparison.get("version"), COMPARISON_VERSION):
    ...
    if manifest.get("format") != FIDELITY_FORMAT or not _is_version(manifest.get("version"), FIDELITY_VERSION):
...
def read_review_by_package(
    ...
    *,
    ...
    if set(receipt) != expected_fields or receipt.get("format") != FORMAT or not _is_version(receipt.get("version"), VERSION):
