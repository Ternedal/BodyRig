...
def evaluate_rulesets(rulesets: list[dict[str, Any]] | None) -> dict[str, Any]:
    applicable = [r for r in (rulesets or []) if isinstance(r, dict) and _ruleset_applies_to_main(r)]
    if not applicable:
        return {
            "mode": "ruleset",
            "passed": False,
...
def evaluate_repository_authority(
    ...
    *,
    ...
    if not passing_modes:
        errors.append("neither classic branch protection nor an equivalent active ruleset satisfies BodyRig repository authority")

    selected = passing_modes[0] if passing_modes else None
    return {
        "format": FORMAT,
        "version": VERSION,
        "repository": "Ternedal/BodyRig",
        "branch": "main",
        "head_revision": head,
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Validate BodyRig GitHub main repository authority.")
