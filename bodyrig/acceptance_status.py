...
def _session_status(session_path: Path) -> AcceptanceStatus:
    ...
    if session.get("format") != "bodyrig-physical-clone-session" or not _is_version(session.get("version"), 1):
...
def _validate_gate_a(path: Path) -> GateAInfo:
    ...
    if report.get("format") != "bodyrig-rig-acceptance" or not _is_version(report.get("version"), 1):
...
def _validate_probe(path: Path, *, platform: str, gate: GateAInfo) -> dict[str, Any]:
    ...
    if probe.get("format") != "bodyrig-renderer-probe" or not _is_version(probe.get("version"), 1) or probe.get("platform") != platform:
...
def _validate_deformation(path: Path, *, platform: str, probe: dict[str, Any], gate: GateAInfo) -> None:
    ...
    if deformation.get("format") != "bodyrig-deformation-probe" or not _is_version(deformation.get("version"), 1) or deformation.get("platform") != platform:
...
def _validate_attestation(path: Path, *, platform: str, gate: GateAInfo, paths: PlatformPaths) -> None:
    ...
    if attestation.get("format") != "bodyrig-renderer-acceptance" or not _is_version(attestation.get("version"), 1):
...
def _validate_release_artifact(
    ...
    *,
    ...
    if release.get("format") != "bodyrig-release-acceptance" or not _is_version(release.get("version"), 1):
