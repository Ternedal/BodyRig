...
def _accessor_values(
    ...
    *,
    ...
    if view_end > len(binary):
        raise HandsFeetNailsUvDomainEvidenceError(f"{label} bufferView exceeds GLB binary bytes")
    ...
    for item in range(count):
        ...
        result.append(tuple(unpack.unpack(binary[begin:end])))
    return result
...
def _canonical_mesh(document: Mapping[str, Any]) -> tuple[int, int, int, dict[str, Any], list[int], list[str]]:
    ...
    mesh_matches = [index for index, item in enumerate(meshes) if isinstance(item, dict) and item.get("name") == MESH_NAME]
    if len(mesh_matches) != 1:
        raise HandsFeetNailsUvDomainEvidenceError("canonical source-derived body mesh is missing or ambiguous")
    ...
    if not isinstance(primitives, list) or len(primitives) != 1 or not isinstance(primitives[0], dict):
        raise HandsFeetNailsUvDomainEvidenceError("canonical source-derived body mesh must contain one primitive")
    primitive = primitives[0]
    attrs = primitive.get("attributes")
    required = {"TEXCOORD_0", "JOINTS_0", "WEIGHTS_0"}
    if not isinstance(attrs, dict) or not required <= set(attrs):
        raise HandsFeetNailsUvDomainEvidenceError("canonical source-derived body primitive lacks UV/skinning attributes")
    ...
    if len(bound_nodes) != 1:
        raise HandsFeetNailsUvDomainEvidenceError("canonical source-derived body mesh is not bound exactly once to SMPL-X skin")
...
def _region_domain(
    *,
    ...
    if missing:
        raise HandsFeetNailsUvDomainEvidenceError(
            f"{capture_region} target joints are missing: {', '.join(missing)}"
        )
...
def validate_uv_domain_evidence(value: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if value.get("mesh_name") != MESH_NAME or value.get("skin_name") != SKIN_NAME:
    ...
    for capture_region in REGION_JOINT_NAMES:
        ...
        if item.get("target_joint_names") != list(REGION_JOINT_NAMES[capture_region]):
        ...
        if not isinstance(indices, list) or not indices or any(isinstance(index, bool) or not isinstance(index, int) or index < 0 for index in indices):
