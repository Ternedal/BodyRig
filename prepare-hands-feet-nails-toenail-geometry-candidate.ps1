...
if ($dirty.Count -gt 0) {
    throw "HFN toenail geometry preparation requires a clean BodyRig checkout."
}

& $BodyRigPython -m bodyrig.hands_feet_nails_toenail_geometry_candidate_cli `
    --root $Root `
    --person-id $PersonId `
...
if ($LASTEXITCODE -ne 0) {
    throw "BodyRig HFN toenail geometry candidate preparation failed."
}
