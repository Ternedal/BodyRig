...
Selected observation segments keep their full selected time window and original spatial resolution, but are materialized at 15 fps before the shared recovery/identity pipeline consumes them.

The goal is to reduce redundant PHALP/HMR2 frame work. On 30 fps source material the recovery frame count is expected to be roughly halved; on 60 fps material it can be reduced by roughly 75 percent. Those are frame-count implications, not promised wall-clock speedups.
...
- Built-in identity capture samples source media by timestamp at 0.75 second intervals, so a 15 fps segment still contains substantially denser temporal evidence than the identity sampler consumes.
