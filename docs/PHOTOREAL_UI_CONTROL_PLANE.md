...
The same read-only mirror now includes a bounded history of recent performer-bound Photoreal P0 runs. Candidate discovery reuses the artifact performer declarations. Exactly the latest valid run used by the control-plane status is marked as the continuation candidate; older valid runs are explicitly history-only and never gain execution authority. History shows stable local evidence such as P0 status presence, calibration integrity/authorization state, teacher-root/input/config/manifest presence and a performer-bound teacher-input SHA-256 when valid.

Run discovery also performs a bounded integrity audit of same-prefix candidates. Symlinked run roots, non-directory candidates, missing trusted performer declarations, conflicting performer declarations and performer mismatches are surfaced as explicit `REJECTED` history rows instead of disappearing silently. Rejected entries carry no teacher/workspace probing, no continuation flag, no historical execution authority and no production authority. Their timestamps come from the candidate path's own `lstat` metadata, so a symlink target is never followed even for display metadata. If the newest same-prefix candidate is rejected, Drift raises top-level Photoreal attention and shows the rejection reason under **Hvorfor?**, even when an older valid ExAvatar run is still busy. A no-valid-run state can therefore still expose rejected local evidence without turning it into execution authority.
