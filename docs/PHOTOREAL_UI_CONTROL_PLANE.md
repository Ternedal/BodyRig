...
Only strict M6 readback may show `digital_twin_ready=true` and `production_activation=true`. Missing, invalid or ambiguous evidence remains required/blocked. Drift exposes one narrow typed mutation: `advance-m5` for machine-executable Windows/Quest M5 realization only. The browser receives an id/platform/label action catalog, never the canonical command. On click the backend strict-recomputes the complete current M4 → physical acceptance → M5 status and launches only when the gate is still `digital_twin_platform_acceptance`, the platform is one of the two canonical M5 targets and the existing operator-status engine authorizes an exact command from the clean evidence checkout. Drift does not auto-record human or physical PASS and does not finalize or activate M6.
...
This downstream pane exposes only state, canonical authority ids, already-whitelisted evidence directories, messages and a command-free typed M5 action catalog. The action endpoint can launch the next canonical Windows/Quest machine probe through the restart-safe operator supervisor, but it has no platform attestation capability, no human/physical PASS authority and no M6 finalization/activation authority. A gate change between render and click is revalidated and rejected. Missing platform status or a failed downstream strict read stays blocked instead of being inferred from file presence. M5 launches are persisted under the `digital-twin` operator-launch category with Person, revision, platform and M4 authority context for audit.
