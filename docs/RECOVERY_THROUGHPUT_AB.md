...
The wrapper binds both Git HEAD and the imported `bodyrig.recovery_throughput_sampling_audit` module to that exact checkout before evaluating evidence. A stale/global/editable BodyRig installation cannot authorize the comparison.
...
After machine PASS, remain on the same exact clean candidate checkout:
...
The wrapper re-runs the machine gate before copying any review bytes and binds the review-bundle Python module to the same exact checkout. The bundle contains the canonical baseline/candidate views, `machine-audit.json`, `index.html`, and a SHA-256 manifest. Existing output is never overwritten.
...
Remain on the exact clean candidate checkout whose revision is recorded in `review-bundle.json`. The wrapper refuses to record a review if current HEAD differs from the bundle's candidate revision, if the checkout is dirty, or if the human-review Python module imports from another checkout.
