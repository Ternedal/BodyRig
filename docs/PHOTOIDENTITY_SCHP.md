...
- `hair-detail`: may contribute only to `hair_hairline` when a real Hair mask is adjacent to a real Face mask and source-space extent/quality thresholds pass.
...
SCHP/ATR has **no authority** to prove subject-specific anatomy, rear orientation, breasts/torso shape, waist/hips shape, fingernails or toenails. Those remain separate blockers until purpose-built source evidence exists.
