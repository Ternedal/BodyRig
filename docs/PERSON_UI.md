...
When Drift has current operator-attention items, the HUD attention signal opens Live Activity first instead of forcing the operator directly into the full Drift tab. Live Activity now consumes versioned, bounded structured snapshots published by Drift on the already-rendered attention/job/launch/Photoreal nodes; it no longer clones source rows or parses rendered labels/detail text. Missing, overlong, control-character-bearing, unknown-kind or unknown-state snapshots fail closed and are omitted from the drawer. Attention active/unseen counts are likewise accepted only from the versioned structured attention badge, while the selected Person name comes from the versioned global HUD snapshot.

Attention navigation still proxies only the already-existing source navigation button, using the structured action label rather than button text. If the source item has been replaced by a refresh, the drawer refreshes instead of invoking a stale action. Persisted jobs and operator launches expose navigation-only **Åbn i Drift** drill-downs keyed by their structured activity kind/id. The drawer resolves the exact currently rendered Drift row before scrolling and never mirrors or exposes cancel, VoiceRig choice, operator execution, audio preview, expanded evidence/logs or any other mutation control. Photoreal Live Activity consumes only the bounded structured Photoreal title/state/detail snapshot; it never reads the visible badge or preformatted diagnostic text as authority.
