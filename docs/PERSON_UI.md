...
Personality-fanens sticky control strip er structured-state consumer. Person Profile-renderingen publicerer kandidatantal og aktiv canonical personality-binding, mens Personality workspace publicerer den aktuelle Guided/Audition lab-mode som eksplicit state. Strippen parser ikke længere revision-DOM, `personalityWorkspaceStatus` eller `Personlighed …`-pillens tekst for readiness.
