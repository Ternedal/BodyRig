...
Personality-fanens sticky control strip er nu en ren structured-state consumer. `person_app.js` publicerer valgt `person_id`, kandidatantal og aktiv canonical personality-binding på `personalityControlStrip.dataset`, mens `personality_workspace.js` publicerer Guided/Audition mode, readiness, label og den Person-id som iframe-workspace faktisk er bundet til. Strippen accepterer kun versioneret state og kræver, at `labPersonId` matcher den aktuelt valgte Person, før Personality Lab kan være `ready`. Kandidat-DOM, `personalityWorkspaceStatus`-tekst og `Personlighed …`-pillens label bruges ikke længere som readiness authority. Ved personskift eller malformed/delvis state vises `Ukendt` fail-closed, indtil workspace-bindingen igen matcher den valgte Person.
...
Body-fanen loader nu hele den eksisterende **High-fidelity Body-kæde**: source review-preview, component visual review, hair deformation review, hair promotion og continuation. Modulerne monteres i den rækkefølge, deres eksisterende canonical gates forventer. Body-fanen loader nu også den eksisterende **High-fidelity continuation**-flade. Den finder den persisted high-fidelity preview for den valgte body-revision, viser component/package/gate-status og kan kun starte den næste gate gennem den allerede eksisterende typed `/continuation-action` backend. Operator-input-gates forbliver eksplicitte; production vises kun som klar, når canonical continuation-status både rapporterer `production_ready=true` og `production_activation=true`. UI-wiringen introducerer ingen ny authority-kontrakt.
...
Saml person-fanens sticky control strip er state-båret. `person_app.js` publicerer antal valgte komponenter, assembly/audition lifecycle (`idle` / `running` / `ready`) og review-lock direkte fra den canonical `state.assembly` + `auditionReady()` logik. Strippen parser ikke fingerprint-, body-, voice-, personality- eller review-badges for at afgøre readiness.
...
The command palette is a true keyboard modal: opening it captures the previously focused element, focus is trapped inside the palette while it is open, Escape/backdrop close restore focus when the original element still exists, and the search input exposes combobox/listbox semantics with `aria-activedescendant` synchronized to the highlighted command. The command palette exposes **Kræver handling** only while the current Drift attention badge is non-zero. It routes to the same Live Activity surface. The palette now also consumes the same versioned structured HUD and Mission Control snapshots as the other global control-room layers: selected-person context comes from `personHud.dataset`, not the rendered person-name label, and **Næste handling** is present only when Mission Control reports a valid structured `attention` or `next` state with an allow-listed target tab. The palette revalidates that snapshot again when the command is executed, so a stale or malformed Mission target becomes a no-op instead of navigation authority. `unknown` and `complete` mission states expose no next-action command.


Drift now also includes a read-only **Motor State v3** monitor sourced from the canonical `GET /api/v3/runtime/motor-state` endpoint on the existing Drift refresh cadence. The card validates `type=bodyrig-motor-state`, `version=3`, bounded core motion values and bounded optional performed-state fields before rendering motion, expression, gesture, gaze, posture, locomotion and speech. Invalid or unavailable state fails closed as unknown. Raw `embodiment.observed` evidence is deliberately not rendered or treated as a second personalization/runtime authority, and the monitor exposes no cue/action endpoint.
