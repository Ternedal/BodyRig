...
Write-Host "After reviewing all four canonical views, record the real human decision explicitly:"
Write-Host "  .\record-recovery-throughput-human-review.ps1 -BundleDir '$finalBundle' -IdentityShape pass|fail -FaceIdentity pass|fail -SkinTextureAlignment pass|fail -GrossAnatomy pass|fail -Note '<human note>' -Out '$humanOut'"
Write-Host "Authority: comparison-only; human decision still required; no promotion/production activation."
