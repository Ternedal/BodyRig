...
function Get-VerifierProvenance {
    ...
    if ($revision -notmatch '^[0-9a-f]{40}    $expectedBlobLines = @(& git -C $repoRoot rev-parse "${revision}:$relativePath" 2>&1)
...
function Test-HistoricalRevisionAncestor {
    ...
    $exists = @(& git -C $RepoRoot cat-file -e "$EvidenceRevision^{commit}" 2>&1)
