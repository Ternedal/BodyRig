...
if([string]$a.format-ne'bodyrig-rig-acceptance'-or -not (Test-V1Version $a.version)-or$a.automated_pass-ne$true-or$a.bodyrig_checkout_clean-ne$true-or[string]$a.physical_renderer_acceptance-ne'pending'-or$a.production_activation-ne$false){throw 'Automated acceptance is not a valid pending-renderer PASS.'}
...
$visualGuard=Join-Path $root 'assert-runtime-visual-authority.ps1';if(-not(Test-Path -LiteralPath $visualGuard -PathType Leaf)){throw "Runtime visual authority guard not found: $visualGuard"};& $visualGuard -AcceptanceDir $dir
...
if([string]$skinQa.Value.format-ne'bodyrig-skin-qa'-or -not (Test-V1Version $skinQa.Value.version)-or[string]$skinQa.Value.body_id-ne$bodyId-or(Need-Sha ([string]$skinQa.Value.package_sha256) 'skin QA package')-ne$packageHash-or(Need-Sha ([string]$skinQa.Value.avatar_sha256) 'skin QA avatar')-ne$avatarHash-or$skinQa.Value.structural_pass-ne$true-or$skinQa.Value.manual_review_required-ne$true-or[string]$skinQa.Value.automated_assessment-ne[string]$a.skin_qa.automated_assessment){throw 'Anatomical skin QA no longer matches accepted package/avatar or Gate A.'}
...
function Read-Probe([string]$Path,[string]$Platform){
    ...
    if(@(Compare-Object $fields @($v.PSObject.Properties.Name)).Count-ne0-or[string]$v.format-ne'bodyrig-renderer-probe'-or -not (Test-V1Version $v.version)-or[string]$v.platform-ne$Platform){throw "Invalid renderer probe: $($f.Path)"}
...
function Read-Deformation([string]$Path,[string]$Platform,$Probe){
    ...
    if(@(Compare-Object $fields @($v.PSObject.Properties.Name)).Count-ne0-or[string]$v.format-ne'bodyrig-deformation-probe'-or -not (Test-V1Version $v.version)-or[string]$v.platform-ne$Platform){throw "Invalid deformation probe: $($f.Path)"}
...
function Read-Att([string]$Path,[string]$Platform,$Probe,$Deformation){
    ...
    if([string]::IsNullOrWhiteSpace([string]$v.quality_note)){throw 'Renderer quality note missing.'}
    $qualityNote=([string]$v.quality_note).Trim();if($qualityNote-match'^<[^>]+>$'){throw "$Platform renderer quality note is still a generated placeholder."};$f
}
