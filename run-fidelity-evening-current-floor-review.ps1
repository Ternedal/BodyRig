...
$faceSecondaryPreview = Need-File -Path (Join-Path $repoRoot "run-face-secondary-hair-eye-windows-preview.ps1") -Label "Face-secondary on hair+eye preview runner"
...
$retainedVisibilityPath = Need-File -Path (Join-Path $retainedOutput "windows-preview\component-visibility-probe.json") -Label "Retained component visibility probe"
...
if ([string]$retainedGap.package_sha256 -ne $currentPackageSha -or [string]$retainedGap.bodyrig_revision -ne $head) { throw "Component gap authority targets different current-floor bytes/revision." }
