...
def test_checkpoint_identity_rejects_historical_uncapped_payload() -> None:
    payload = {
        "format": bridge.CHECKPOINT_FORMAT,
        "version": bridge.CHECKPOINT_VERSION,
        "adapter": ADAPTER_NAME,
        "revision": ADAPTER_REVISION,
        "source_index": 0,
        "source_sha256": "a" * 64,
    }
    assert bridge._checkpoint_identity_matches(
        payload,
        source_index=0,
        source_sha256="a" * 64,
        sampling_stride=2,
        format_name=bridge.CHECKPOINT_FORMAT,
    ) is False

    payload["sampling_policy"] = RECOVERY_TEMPORAL_SAMPLING_POLICY
    payload["sampling_stride"] = 1
    assert bridge._checkpoint_identity_matches(
        payload,
        source_index=0,
        source_sha256="a" * 64,
