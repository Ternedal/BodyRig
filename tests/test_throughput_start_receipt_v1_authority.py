...
def test_wrapper_machine_result_uses_bool_safe_numeric_v1() -> None:
    source = WRAPPER.read_text(encoding="utf-8")
    assert "function Test-V1Version($Value)" in source
    assert "Test-V1Version $candidateResult.version" in source
    assert "[int]$candidateResult.version" not in source
    assert '[string]$candidateResult.format -eq "bodyrig-throughput-candidate-run-plan"' in source
    assert '$machineResults.Count -ne 1' in source
    assert '[string]$started.person_id -ne [string]$gateBefore.person_id' in source
    assert 'Try-CancelCandidateJob -JobId $candidateJobId' in source
    assert 'physical_acceptance_authority = $false' in source
...
def test_throughput_start_v1_helper_runtime_semantics(path: Path) -> None:
