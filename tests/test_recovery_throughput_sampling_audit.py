...
def test_machine_gate_accepts_only_exact_source_segment_track_and_sampling_derivative() -> None:
    result = compare_runs(
...
def test_arbitrary_recovery_change_is_blocked_even_when_faster() -> None:
    result = compare_runs(
        _run(job_id="baseline", revision=BASELINE_REVISION, frames=3600),
        _run(job_id="candidate", revision=BASELINE_REVISION + ";x:other", frames=900),
    )
...
def test_current_succeeded_ui_job_schema_does_not_require_legacy_performer_or_package_fields(tmp_path: Path) -> None:
    job = {
