...
def test_gate_a_requires_windows_ps7_before_transactional_staging() -> None:
    ...
    git_binding = source.index("$head = Assert-CheckoutAuthority -RepoRoot $repoRoot")
    staging = source.index('$staging = Join-Path $outputParent')

    assert windows_gate < version_gate < pwsh_resolution < repo_binding < git_binding < staging
...
def test_gate_a_core_retains_exact_checkout_binding_before_acceptance_output() -> None:
    ...
    assert repo_binding < git_binding < output_creation
    assert "BodyRig checkout is dirty; high-fidelity Gate A requires the exact clean clone revision." in source


def test_gate_a_has_no_legacy_powershell_fallback() -> None:
    wrapper = (REPO / "accept-physical-clone.ps1").read_text(encoding="utf-8")
    core = (REPO / "accept-physical-clone-core.ps1").read_text(encoding="utf-8")
