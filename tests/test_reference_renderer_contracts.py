...
def test_build_script_has_physical_windows_and_quest_targets() -> None:
    ...
    assert "XRGeneralSettingsPerBuildTarget.XRGeneralSettingsForBuildTarget(BuildTargetGroup.Android)" in source
    assert "XRPackageMetadataStore.AssignLoader" in source
    assert "XRGeneralSettingsPerBuildTarget.SettingsForBuildTarget(BuildTargetGroup.Android)" not in source
