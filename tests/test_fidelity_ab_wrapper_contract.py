...
def test_fidelity_ab_wrapper_is_strict_and_checkout_bound() -> None:
    ...
    assert "[string]$ExpectedLeftBuilderRevision" in text
    assert "[string]$ExpectedRightBuilderRevision" in text
    assert "bodyrig.fidelity_ab" in text
    assert "bodyrig.fidelity_ab_cli" in text
    assert "--expected-left-builder-revision" in text
    assert "--expected-right-builder-revision" in text
