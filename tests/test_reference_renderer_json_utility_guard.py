...
def test_raw_guard_runs_for_v1_v2_v3_before_unity_erases_types() -> None:
    ...
    assert 'RequireStringField(json, "body_id", "root");' in shim
    assert 'RequireStringField(json, "utterance_id", "root");' in shim
...
def test_shared_action_objects_fail_closed_before_deserialization() -> None:
    ...
    assert "foreach (var field in stringFields)" in source
    assert "RequireStringField(body, field, context);" in source
