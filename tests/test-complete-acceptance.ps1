...
function New-Probe {
    param($Fixture,[ValidateSet("windows-unity-univrm","android-quest-class")][string]$Platform,[string]$Path,[string]$RendererVersion="fixture-renderer-1",[string]$UnityPlatform="",[string]$DeviceModel="",[string]$BodyRigRevision="")
...
function Invoke-RendererRecord {
    ...
    $args=@("-NoLogo","-NoProfile","-NonInteractive","-File",$Fixture.RecorderScript,"-AcceptanceReport",$Fixture.ReportPath,"-RuntimeManifest",$Fixture.RuntimeManifestPath,"-ProbeReport",$ProbePath,"-DeformationReport",$DeformationPath,"-Platform",$Platform,"-Pass","-ConfirmQualityChecklist","-RendererName","BodyRig Reference Renderer","-RendererVersion",$RendererVersion,"-QualityNote",$QualityNote,"-Output",$Output)
...
try {
    ...
    if([string]$wAtt.quality_review.revision-ne"bodyrig-human-quality-v1"){throw "renderer attestation did not bind structured human quality revision"};foreach($field in @("full_deformation_sequence_reviewed","source_identity_texture_acceptable","geometry_proportions_acceptable","upper_body_deformation_acceptable","lower_body_deformation_acceptable","cross_limb_leakage_absent","skin_qa_considered")){if($wAtt.quality_review.$field-ne$true){throw "renderer attestation quality field not true: $field"}}
    ...
    $f=New-Fixture "wrong-build-revision";$p=New-Probe $f "windows-unity-univrm" (Join-Path $f.Artifacts "probe.json") -BodyRigRevision ("0"*40);Assert-Failure (Invoke-RendererRecord $f "windows-unity-univrm" $p (Join-Path $f.Artifacts "att.json")) "renderer build revision mismatch";Write-Host "PASS: machine probe from different BodyRig build revision rejected"
    $f=New-Fixture "deformation-revision";$p=New-Probe $f "windows-unity-univrm" (Join-Path $f.Artifacts "probe.json");$d=New-Deformation $f "windows-unity-univrm" $p (Join-Path $f.Artifacts "deformation.json");$j=Get-Content $d -Raw|ConvertFrom-Json;$j.bodyrig_revision="0"*40;$j|ConvertTo-Json -Depth 10|Set-Content $d -Encoding UTF8;Assert-Failure (Invoke-RendererRecord $f "windows-unity-univrm" $p (Join-Path $f.Artifacts "att.json") -DeformationPath $d) "deformation build revision mismatch";Write-Host "PASS: deformation from different BodyRig build revision rejected"
    ...
    $f=New-Fixture "quality-review-tamper";$r=New-RendererPair $f;$j=Get-Content $r.Windows -Raw|ConvertFrom-Json;$j.quality_review.cross_limb_leakage_absent=$false;$j|ConvertTo-Json -Depth 10|Set-Content $r.Windows -Encoding UTF8;Assert-Failure (Invoke-PairGate $f $r) "human quality review tamper";Write-Host "PASS: human quality review tamper rejected"
    ...
    Write-Host "BodyRig structured-human-quality + high-fidelity skin-QA + exact-build-provenance + deformation-bound physical-device release gate tests: PASS";exit 0
