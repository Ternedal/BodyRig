...
function Invoke-RendererRecord {
    param($Fixture,[ValidateSet("windows-unity-univrm","android-quest-class")][string]$Platform,[string]$ProbePath,[string]$Output,[string]$QualityNote="Avatar loads correctly and fixed deformation sweep looked visually plausible.",[string]$RendererVersion="fixture-renderer-1")
...
function New-RendererPair {
    ...
    $wr=Join-Path $Fixture.Artifacts "windows-renderer.json";$qr=Join-Path $Fixture.Artifacts "quest-renderer.json";Assert-Success (Invoke-RendererRecord $Fixture "windows-unity-univrm" $wp $wr) "record Windows";Assert-Success (Invoke-RendererRecord $Fixture "android-quest-class" $qp $qr) "record Quest"
...
try {
    ...
    Write-Host "BodyRig high-fidelity skin-QA + deterministic physical deformation final release gate tests: PASS";exit 0
