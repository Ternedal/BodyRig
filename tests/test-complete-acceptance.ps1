...
function New-Fixture {
    ...
    $avatarBytes=$utf8.GetBytes("fixture-avatar-vrm-bytes-$Name"); $bodyprintBytes=$utf8.GetBytes('{"format":"modelrig-bodyprint","version":1,"shape":{"shoulder_to_height":0.24}}'); $provenanceBytes=$utf8.GetBytes('{"format":"modelrig-body-provenance","version":1}'); $thumbnailBytes=[byte[]](0x89,0x50,0x4e,0x47,0x0d,0x0a,0x1a,0x0a,0x66,0x69,0x78)
    ...
    $report=[ordered]@{format="bodyrig-rig-acceptance";version=1;created_at=[DateTime]::UtcNow.ToString("o");bodyrig_revision=$head;bodyrig_checkout_clean=$true;source_count=1;recovery=[ordered]@{adapter="4dhumans-hmr2-phalp";revision="fixture-pinned";track_id="track-1";observed_frames=120};package=[ordered]@{package_sha256=$packageHash;body_id=$bodyId;body_name="Fixture Body";payload_names=@("avatar.vrm","bodyprint.json","provenance.json","thumbnail.png");bodyprint_matches_proof=$true;source_count_matches=$true;recovery_provenance_matches=$true;avatar_fitting_provenance_present=$true;vrm_spec_version="1.0";placeholder_avatar=$true};runtime=[ordered]@{manifest="runtime/runtime-manifest.json";manifest_sha256=$runtimeManifestHash;materialized_from_package=$true};checks=$checks;automated_pass=$true;physical_renderer_acceptance="pending";production_activation=$false}
...
try {
    ...
    Write-Host "BodyRig physical-device-bound final release gate tests: PASS";exit 0
