...
def test_wrapper_executes_sampling_audit_from_exact_candidate_checkout() -> None:
    assert 'Push-Location -LiteralPath $RepoRoot' in SCRIPT
    assert '"bodyrig\\recovery_throughput_sampling_audit.py"' in SCRIPT
    assert 'bodyrig.recovery_throughput_sampling_audit' in SCRIPT
    assert 'importlib.import_module(sys.argv[1])' in SCRIPT
    assert 'Recovery throughput audit imported BodyRig from a different checkout' in SCRIPT
    assert '[Environment]::SetEnvironmentVariable("PYTHONPATH", $boundPythonPath, "Process")' in SCRIPT
    assert '[Environment]::SetEnvironmentVariable("PYTHONDONTWRITEBYTECODE", "1", "Process")' in SCRIPT
    assert '[Environment]::SetEnvironmentVariable("PYTHONPATH", $previousPythonPath, "Process")' in SCRIPT
    assert 'if ($locationPushed) { Pop-Location }' in SCRIPT
