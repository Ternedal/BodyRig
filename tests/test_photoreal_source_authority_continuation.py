...
SCRIPTS = (
    ROOT / "run-photoreal-p0-windows.ps1",
    ROOT / "resume-photoreal-v2-after-source.ps1",
)
...
def test_uncalibrated_identity_does_not_preempt_source_authority(script: Path) -> None:
    ...
    assert "source-authoritative single-performer bindings are resolved independently" in block
    assert "Ambiguous sources without calibrated matching will remain unresolved" in block
    assert calibration < frame_measure < frame_authority < frame_index


@pytest.mark.parametrize("script", SCRIPTS)
def test_calibration_still_persists_advisory_evidence(script: Path) -> None:
    source = script.read_text(encoding="utf-8")
...
@pytest.mark.parametrize("script", SCRIPTS)
def test_source_authority_continuation_scripts_parse_when_pwsh_available(
    script: Path,
) -> None:
