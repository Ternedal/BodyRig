...
def _profile(root: Path) -> dict:
    return create_profile(root, display_name="Source Fixture", stash_performer={"id": "42", "name": "Source Fixture", "disambiguation": ""})


def _components(root: Path, person_id: str) -> dict:
    add_body_revision(root, person_id, body_id="fixture-body", package_sha256="a" * 64, package_path=str(root / "fixture.mrbody"))
    add_voice_revision(root, person_id, voice_id="fixture-voice", voice_package="fixture.mrvoice", package_sha256="b" * 64)
    return add_personality_revision(root, person_id, instructions="Be the source-bound fixture.", default_language="en")


def _bind_body_and_personality(root: Path, profile: dict) -> None:
    write_binding(root, profile, kind="body", revision_id="body-r0001", evidence_kind="test-body-source-v1", evidence_sha256="c" * 64, evidence_ref="fixture-body-source")
    write_binding(root, profile, kind="personality", revision_id="personality-r0001", evidence_kind="test-personality-source-v1", evidence_sha256="d" * 64, evidence_ref="fixture-personality-source")


def test_source_backed_voice_revision_is_bound_to_exact_package(tmp_path: Path) -> None:
    profile = _profile(tmp_path)
    profile = add_voice_revision(tmp_path, profile["person_id"], voice_id="fixture-voice", voice_package="fixture.mrvoice", package_sha256="b" * 64)
    receipt = read_binding(tmp_path, profile, kind="voice", revision_id="voice-r0001")
    assert receipt["source"]["performer_id"] == "42"
    assert receipt["component"]["artifact_sha256"] == "b" * 64
    assert receipt["evidence"] == {"kind": "voicerig-package-v1", "sha256": "b" * 64, "ref": "fixture.mrvoice", "source_files": []}
...
def test_source_backed_profile_exposes_blockers_and_assembly_fails_before_audition(tmp_path: Path) -> None:
    ...
    write_binding(tmp_path, profile, kind="body", revision_id="body-r0001", evidence_kind="test-body-source-v1", evidence_sha256="c" * 64, evidence_ref="fixture-body-source")
    ...
    with pytest.raises(PersonAssemblyError, match="personality-r0001.*source binding missing"):
        build_assembly(profile, body_revision="body-r0001", voice_revision="voice-r0001", personality_revision="personality-r0001")
    with pytest.raises(PersonProfileError, match="personality.*source binding missing"):
        add_person_revision(tmp_path, profile["person_id"], body_revision="body-r0001", voice_revision="voice-r0001", personality_revision="personality-r0001", compatibility_review=REVIEW, activate=False)
...
def test_source_backed_person_revision_and_activation_require_all_bindings(tmp_path: Path) -> None:
    ...
    assembly = build_assembly(profile, body_revision="body-r0001", voice_revision="voice-r0001", personality_revision="personality-r0001")
    assert len(assembly["assembly_fingerprint"]) == 64
    profile = add_person_revision(tmp_path, profile["person_id"], body_revision="body-r0001", voice_revision="voice-r0001", personality_revision="personality-r0001", compatibility_review=REVIEW, activate=False)
