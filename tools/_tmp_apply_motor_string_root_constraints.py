...
'''
    new = '''            RequireStringMember(fields, "state", "speech");
            RequireIntegerMember(fields, "elapsed_ms", "speech");
...
'''
    ...
    old = '''            var fields = ParseObjectMembers(raw, "posture");
            var id = RequireStringMember(fields, "id", "posture");
            if (fields.ContainsKey("source"))
'''
    new = '''            var fields = ParseObjectMembers(raw, "posture");
...
def _string_schema(path: Path, *keys: str) -> dict:
    node = _contract(path)
    for key in keys:
        node = node[key]
    return node
...
def test_root_allowlists_match_canonical_version_properties() -> None:
    ...
    assert "RootFieldsForVersion" in validate
...
def test_posture_id_constraint_matches_all_generic_schema_variants() -> None:
    source = SHIM.read_text(encoding="utf-8")
