param(
    ...
    [Parameter(Mandatory=$true)][string]$UvEvidence,
...
& $BodyRigPython -m bodyrig.hands_feet_nails_detail_candidate_cli `
    --root $Root `
    --person-id $PersonId `
    --body-revision $BodyRevision `
    --capture-id $CaptureId `
    --uv-evidence $UvEvidence `
    --package $PackagePath `
    --bodyrig-revision $head

if ($LASTEXITCODE -ne 0) {
    throw "BodyRig HFN detail candidate preparation failed."
}
