...
if ($LASTEXITCODE -ne 0 -or $dirty.Count -gt 0) { throw "Anatomy source discovery requires an exact clean BodyRig checkout." }
...
if ($evidenceRevision -ne $head) {
    throw "Photoidentity sweep belongs to BodyRig revision $evidenceRevision, current checkout is $head. Refusing cross-revision anatomy evidence."
}
...
Write-Host "BodyRig anatomy source discovery preflight"
Write-Host "Revision: $head"
Write-Host "Sweep:    $SweepRoot"
Write-Host "Policy:   source crops only; machine anatomy/rear authority FALSE; no avatar render"
Write-Host ""
...
& $BodyRigPython -m bodyrig.photoidentity_anatomy_source_discovery `
  --sweep-root $SweepRoot `
  --ffmpeg $Ffmpeg `
  --distribution $distribution `
  --openpose $openpose `
  --wsl-exe $wslExe
if ($LASTEXITCODE -ne 0) { throw "BodyRig anatomy source discovery failed with exit code $LASTEXITCODE." }

Write-Host ""
Write-Host "BodyRig anatomy source discovery: COMPLETE"
Write-Host "No rear/torso/waist sufficiency authority has been granted. Review source crops before any PASS can exist."
