...
try {
    ...
    # Validate every Gate-A acceptance structurally first and rank by the
    # furthest valid physical gate, not by job timestamp. Historical evidence
    # is ranked using its own revision-bound structure; operator/reference policy
    # is applied only after the selected exact revision is active.
    ...
    foreach ($candidate in $acceptanceCandidates) {
        ...
        $assessmentRaw = @(& $python -m bodyrig.rig_window_acceptance $candidate.acceptance_dir 2>&1)
        if ($LASTEXITCODE -ne 0) { continue }
        try { $acceptance = ($assessmentRaw -join "`n") | ConvertFrom-Json }
        catch { continue }
        if ([string]$acceptance.state -eq "error" -or [int]$acceptance.progress_rank -le 0) { continue }
        $acceptanceAssessments += [pscustomobject]@{
            job_id = [string]$candidate.job_id
            acceptance_dir = [string]$candidate.acceptance_dir
            stamp = [string]$candidate.stamp
            state = [string]$acceptance.state
            gate = [string]$acceptance.gate
            evidence_revision = ([string]$acceptance.bodyrig_revision).Trim().ToLowerInvariant()
            progress_rank = [int]$acceptance.progress_rank
        }
    }
    $acceptanceAssessments = @(
        $acceptanceAssessments |
            Sort-Object \
                @{ Expression = { [int]$_.progress_rank }; Descending = $true }, \
                @{ Expression = { [string]$_.stamp }; Descending = $true }
    )

    foreach ($candidate in $acceptanceAssessments) {
        if ([string]$candidate.state -eq "complete") {
            $result = [ordered]@{
                format = "bodyrig-rig-window-plan"
                version = 1
                read_only = $true
                ...
                bodyrig_revision = $head
                evidence_revision = [string]$candidate.evidence_revision
                acceptance_dir = [string]$candidate.acceptance_dir
                gate = [string]$candidate.gate
                progress_rank = [int]$candidate.progress_rank
                expensive_reconstruction_rerun = $false
                fitter_rerun = $false
            ...
            if ($Json) { $result | ConvertTo-Json -Depth 5 -Compress }
                ...
                Write-Host "Path: furthest existing acceptance"
                Write-Host "Gate: $([string]$candidate.gate)"
                Write-Host "Evidence revision: $([string]$candidate.evidence_revision)"
        ...
        $evidenceRevision = [string]$candidate.evidence_revision
        if ($evidenceRevision -match '^[0-9a-f]{40}$' -and $evidenceRevision -ne $head) {
            ...
            if ($Json) { $result | ConvertTo-Json -Depth 5 -Compress }
                ...
                Write-Host "Path: furthest historical acceptance exact checkout"
                Write-Host "Gate: $([string]$candidate.gate)"
        ...
        if ($evidenceRevision -ne $head) { continue }
        ...
        if ([string]$status.state -notin @("blocked", "error") -and -not [string]::IsNullOrWhiteSpace([string]$status.next_command)) {
            ...
            if ($Json) { $result | ConvertTo-Json -Depth 5 -Compress }
                ...
                Write-Host "BodyRig rig-window plan: READY | PRIORITY 2"
                Write-Host "Path: furthest existing Gate A acceptance"
