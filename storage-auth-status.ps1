...
function Emit-Status {
    ...
    if ($Json) {
        ...
        if (-not [string]::IsNullOrWhiteSpace($StorageHost)) { Write-Host "Host:       $StorageHost" }
...
if (
    ...
    Emit-Status -State "blocked" -Stage "host-authority" -StorageHost $hostName -Message "Storage config lacks a canonical credential generation or Storage/Stash host authority differs. Re-bootstrap the exact UNC host."
    exit 0
}
...
if ($storage.credential_write_completed -ne $true) {
    ...
    Emit-Status -State "blocked" -Stage "credential-bootstrap-incomplete" -StorageHost $hostName -CredentialPresent $presentDuringInterruptedBootstrap -Message "The previous credential bootstrap did not complete its final authority publication. Re-run bootstrap before any storage proof can be accepted." -NextCommand ".\setup-storage-auth-windows.ps1 -ReplaceExisting"
    exit 0
}

$maxPersist = [int](Get-BodyRigDomainCredentialMaxPersist)
if ($maxPersist -lt 2) {
    Emit-Status -State "blocked" -Stage "windows-policy" -StorageHost $hostName -Message "Windows policy permits persistence level $maxPersist for domain passwords; LOCAL_MACHINE (2) or stronger is required."
    exit 0
}
$credentialPresent = Test-BodyRigDomainCredential -Target $target
if (-not $credentialPresent) {
    Emit-Status -State "required" -Stage "credential-bootstrap" -StorageHost $hostName -CredentialPresent $false -Message "Storage config exists, but the Windows Credential Manager credential is missing." -NextCommand ".\setup-storage-auth-windows.ps1 -ReplaceExisting"
    exit 0
}
...
if (-not (Test-Path -LiteralPath $prePath -PathType Leaf)) {
    ...
    Emit-Status -State "configured" -Stage "fresh-session-proof" -StorageHost $hostName -CredentialPresent $true -Message "Credential is persisted, but a fresh-session Stash-source proof has not been recorded." -NextCommand $next
    exit 0
}
try { $pre = Get-Content -LiteralPath $prePath -Raw -Encoding UTF8 | ConvertFrom-Json }
catch { Emit-Status -State "blocked" -Stage "pre-reboot-proof" -StorageHost $hostName -CredentialPresent $true -Message "Pre-reboot storage proof is unreadable JSON."; exit 0 }
if (
    ...
    Emit-Status -State "blocked" -Stage "pre-reboot-proof" -StorageHost $hostName -CredentialPresent $true -Message "Pre-reboot proof does not match the current credential generation."
    exit 0
}
...
if (Test-Path -LiteralPath $coldPath -PathType Leaf) {
    ...
    catch { Emit-Status -State "blocked" -Stage "cold-boot-proof" -StorageHost $hostName -CredentialPresent $true -Message "Cold-boot proof is unreadable JSON."; exit 0 }
    if (
    ...
    if ($cold.qualified -eq $true -and $passed -ge $required) {
        Emit-Status -State "qualified" -Stage "complete" -StorageHost $hostName -CredentialPresent $true -Passed $passed -Required $required -Message "Persistent SMB authentication has been proven across the required distinct Windows boots with real Stash-source decode and zero prompts."
        exit 0
    }
}
