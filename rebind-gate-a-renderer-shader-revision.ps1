...
if ([System.Environment]::OSVersion.Platform -ne [System.PlatformID]::Win32NT) {
    throw "Renderer shader revision rebind is Windows-only."
}
if ($PSVersionTable.PSVersion.Major -lt 7) {
    throw "PowerShell 7+ (pwsh) is required for renderer shader revision rebind."
}

$repoRoot = (Resolve-Path $PSScriptRoot).Path
$SourceAcceptanceDir = Need-Directory -Path $SourceAcceptanceDir -Label "Source renderer-package-bound Gate A acceptance directory"
$OutputDir = [IO.Path]::GetFullPath($OutputDir)
if (Test-Path -LiteralPath $OutputDir) { throw "Renderer-shader-rebound Gate A output already exists; refusing cross-attempt reuse: $OutputDir" }
...
if ($dirty.Count -gt 0) { throw "BodyRig checkout is dirty; renderer shader revision rebind requires an exact clean checkout." }
...
$sourceAcceptance = Read-Json -Path $sourceAcceptancePath -Label "Source renderer-package-bound Gate A acceptance report"
...
if ([string]::IsNullOrWhiteSpace([string]$sourceAcceptance.physical_clone.renderer_package_revision_rebind_sha256)) { throw "Source Gate A is not the renderer-package-bound acceptance expected before shader repair." }
...
if ($LASTEXITCODE -ne 0) { throw "Could not inspect renderer-shader revision delta." }
$deltaNames = @()
foreach ($line in $deltaLines) {
    if ([string]::IsNullOrWhiteSpace([string]$line)) { continue }
    $parts = ([string]$line) -split "`t"
    if ($parts.Count -ne 2 -or $parts[0] -notin @("A", "M")) { throw "Unsupported renderer-shader rebind revision delta: $line" }
    $deltaNames += $parts[1]
}
...
if (@(Compare-Object -ReferenceObject $expectedNames -DifferenceObject $deltaNames).Count -ne 0) {
    throw "Revision delta is broader than the approved renderer-shader repair/rebind set: $($deltaNames -join ', ')"
}
...
if (-not (Test-Path -LiteralPath $parent -PathType Container)) { throw "Renderer-shader-rebound output parent does not exist: $parent" }
$attempt = Join-Path $parent (".bodyrig-renderer-shader-rebind-" + [Guid]::NewGuid().ToString("N"))
$committed = $false
try {
    ...
    if ($copiedPackage.Count -ne 1 -or (Sha256 $copiedPackage[0].FullName) -ne $ExpectedPackageSha256) { throw "Renderer shader rebind changed package bytes." }
    $copiedRuntimePath = Need-File -Path (Join-Path (Join-Path $attempt "runtime") "runtime-manifest.json") -Label "Copied runtime manifest"
    if ((Sha256 $copiedRuntimePath) -ne $sourceRuntimeHash) { throw "Renderer shader rebind changed runtime-manifest bytes." }
    foreach ($payloadName in @("avatar.vrm", "bodyprint.json")) {
        ...
        if ((Sha256 $sourcePayload) -ne (Sha256 $copiedPayload)) { throw "Renderer shader rebind changed runtime payload bytes: $payloadName" }
    }

    $rebind = [ordered]@{
        format = "bodyrig-renderer-shader-revision-rebind"
        version = 1
    ...
    $rebindPath = Join-Path $attempt "bodyrig-renderer-shader-revision-rebind.json"
    ...
    $acceptance.physical_clone | Add-Member -NotePropertyName renderer_shader_revision_rebind_sha256 -NotePropertyValue $rebindHash -Force
    $acceptance.physical_clone | Add-Member -NotePropertyName renderer_shader_revision_source_bodyrig_revision -NotePropertyValue $ExpectedSourceRevision -Force
    $tempAcceptance = Join-Path $attempt (".bodyrig-acceptance-renderer-shader-rebind-" + [Guid]::NewGuid().ToString("N") + ".tmp")
    try {
    ...
    $check = Read-Json -Path $acceptancePath -Label "Renderer-shader-rebound Gate A acceptance report"
    if (([string]$check.bodyrig_revision).ToLowerInvariant() -ne $head) { throw "Renderer-shader-rebound acceptance revision did not persist." }
    if (([string]$check.package.package_sha256).ToLowerInvariant() -ne $ExpectedPackageSha256) { throw "Renderer-shader-rebound acceptance changed package identity." }
    if (([string]$check.runtime.manifest_sha256).ToLowerInvariant() -ne $sourceRuntimeHash) { throw "Renderer-shader-rebound acceptance changed runtime identity." }
    if (([string]$check.physical_clone.renderer_shader_revision_rebind_sha256).ToLowerInvariant() -ne $rebindHash) { throw "Renderer-shader-rebound acceptance is not bound to rebind evidence." }
...
if ($LASTEXITCODE -ne 0 -or $dirtyAfter.Count -gt 0) { throw "Renderer shader revision rebind did not preserve a clean BodyRig checkout." }
$currentHead = (@(& git -C $repoRoot rev-parse HEAD 2>&1)[0]).Trim().ToLowerInvariant()
if ($currentHead -ne $head) { throw "BodyRig HEAD changed during renderer shader revision rebind." }

Write-Host "BodyRig renderer shader revision rebind: PASS"
