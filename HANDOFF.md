...
- `start-ab-baseline.ps1` is the canonical wrapper when the active #196 PBR and #208 throughput candidates are both receiving fresh physical comparison evidence: before enqueue it runs a service-bound read-only fail-fast preflight against the same service checkout/environment/Python/PATH used by the later body-build, requiring reference-renderer readiness, rig/SiTH/CUDA/Stash readiness and at least one exact-performer `ffmpeg-one-frame-v1` source; it then revalidates the live frozen candidate byte contract, forces baseline workspace retention, revalidates refs after enqueue and publishes the create-only shared baseline plan;
...
- **#196 — ACTIVE/DRAFT CURRENT-MAIN CANDIDATE:** source-derived skin PBR v2. Live authority is PR #196 itself. Before any physical run or merge/promotion decision, require its current head to be exactly one commit ahead / zero behind current `main`, with exactly three changed files whose blobs are `ff79c39f66d44f6649a819da9d75adc35609d0da`, `860a5ece9c92c1438d99b7e32b254ab2457552f1`, and `bb093a213c5b5ddcfd17f5aa2557bd7568d43012`, plus fresh exact-head `ci` and `windows-log-handle-regression` success. CI does **not** grant physical skin/SSS/visual-fidelity or promotion authority. Fresh safe-source baseline/candidate package evidence plus real human visual review is still required.
- **#208 — ACTIVE/DRAFT CURRENT-MAIN CANDIDATE:** PHALP temporal sampling v3. Live authority is PR #208 itself. Before any physical run or merge/promotion decision, require its current head to be exactly one commit ahead / zero behind current `main`, with exactly 18 changed files and the reviewed candidate blobs preserved; in particular the PHALP/HMR2 runtime blobs must remain `853414c46568cfeb8d00fbb00c51acf491d78dbd`, `d310d972b25edec83552babfd2e7e0626698be3e`, and `d8ddd983cc9a6275d424add81ed530326cdf9c0a`, plus fresh exact-head `ci` and `windows-log-handle-regression` success. CI does **not** prove throughput benefit or identity/anatomy preservation. Fresh revision-bound exact-main baseline + exact-candidate body-builds, machine A/B, real human visual review and a later explicit promotion decision remain mandatory.
